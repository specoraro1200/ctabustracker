...
render() {
    ...
    const elements = []
    if(this.state.cta){
        var routes = this.state.cta["routes"]
        for(var i = 0; i < this.state.cta["routes"].length; i++){
            elements.push([routes[i]["rt"],routes[i]["rtnm"]])
        }
        // console.log((this.state.cta)["routes"][0]["rt"])
    }
    ...
    var busRouteNumber = "/BusRoute/" + value[0]
      items.push(<Card ref={this.myRefElement} style={{ width: '18rem' }}>
                    <Card.Img variant="top" src={require("C:/Users/scpec/busaccuracy/myapp/src/bus_route_default.jpg")} />
                    <Card.Body>
                        <Card.Title>
                        <Routes>
                            <Route path="/BusRoute/:id" element={<BusRoute/>}/>
                        </Routes>
                        <a href = {busRouteNumber} >{value[0]+ " " + value[1]}</a>   
                        </Card.Title>
                        <Card.Text>
    ...
    return (
    <Container id = "container" ref={this.container} fluid >
        <div>
        </div>
